...
describe('Mailer Factory', () => {
  ...
  it('should manage path', () => {
    ...
    tree = runner.runSchematic('mailer', options, tree);
    const files: string[] = tree.files;
    expect(
      files.find(filename => filename === '/foo/docker-compose.yml'),
    ).toBeDefined();
    expect(
      files.find(filename => filename === '/foo/templates/partials/layout.handlebars'),
    ).toBeDefined();
    expect(
      files.find(filename => filename === '/foo/templates/views/example.handlebars'),
    ).toBeDefined();
