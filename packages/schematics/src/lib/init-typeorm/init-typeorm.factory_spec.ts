...
describe('Typeorm Factory', () => {
  ...
  it('should work', () => {
    ...
    tree = runner.runSchematic('typeorm', options, tree);
    const files: string[] = tree.files;
    expect(
      files.find(filename => filename === '/docker-compose.yml'),
    ).toBeDefined();
    expect(
      files.find(filename => filename === '/ormconfig.json'),
    ).toBeDefined();
    expect(
  ...
  it('should manage path', () => {
    ...
    const files: string[] = tree.files;
    expect(
      files.find(filename => filename === '/foo/docker-compose.yml'),
    ).toBeDefined();
    expect(
      files.find(filename => filename === '/foo/ormconfig.json'),
    ).toBeDefined();
    expect(
